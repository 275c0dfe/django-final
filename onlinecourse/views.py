...
def extract_answers(request):
    ...
    for test_choice in request.POST.keys():
        if len(test_choice) > 7:
            if test_choice[:7] == "choice_": #Ensure We Are reading a choice

                choice_id = int(request.POST[test_choice]) #Parse Id From dict
